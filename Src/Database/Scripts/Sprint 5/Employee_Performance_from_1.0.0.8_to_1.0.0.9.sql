...
DECLARE @OLD_VERSION VARCHAR(10) = '1.0.0.8'
DECLARE @NEW_VERSION VARCHAR(10) = '1.0.0.9'
...
BEGIN
	...
	BEGIN TRY
		...
		*/
		...
		IF EXISTS(SELECT 1 From INFORMATION_SCHEMA.COLUMNS Where TABLE_NAME = 'TSActivityGroup')
		BEGIN	
			INSERT INTO [dbo].[TSActivityGroup] (Id,ProjectId,Name,Description,CreatedBy,CreatedDate)
			(select  NEWID(), p.Id, p.Name, p.Description, p.CreatedBy, p.CreatedDate
			FROM [dbo].[Project] p  WHERE p.DeletedBy IS NULL AND p.DeletedDate IS NULL)
